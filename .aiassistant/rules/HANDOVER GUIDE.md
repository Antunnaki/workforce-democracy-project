...
Model Transition Inconsistency: System reports using Llama 3.3-70b but has been migrated to Qwen
API Endpoint Mismatch: Async job endpoints returning 404 errors
Configuration Drift: Frontend and backend configurations not aligned
Error Details
...
Impact Areas
API Keys: Need to update from Groq to Qwen keys
Environment Variables: QWEN_MODEL should reflect Qwen model
Health Checks: Should report Qwen model, not Llama
Documentation: All references to Llama should be updated
...
FRONTEND COMPONENTS
Chat Implementation (js/chat-clean.js)
...
Civic Routes (backend/routes/civic-routes.js)
...
FIX RECOMMENDATIONS
Implemented Fixes
Created New Qwen AI Service File: backend/ai-service-qwen.js Issue: No proper Qwen integration existed Fix: Created new service with Qwen API integration

Updated Server Import File: backend/server.js Issue: Still importing old Groq service Fix: Updated to import new Qwen service

Fixed Health Check Response File: backend/routes/civic-routes.js Issue: Reports Llama model instead of Qwen Fix: Updated to correctly report Qwen model

Updated Environment Documentation File: backend/.env.example Issue: Referenced deprecated Groq variables Fix: Added Qwen variables, marked Groq as deprecated

Secondary Fixes Needed
Verify API Key Configuration Issue: Actual Qwen API key needs to be configured on server Action: Add QWEN_API_KEY to server environment

Update Frontend Configuration Flag File: js/config.js Issue: GROQ_ENABLED flag may be misleading Fix: Consider renaming to AI_ENABLED or similar

Restore Async Job Endpoints Issue: Async endpoints may still have issues Investigation needed: Check route registration in server.js

Update Documentation References Issue: Multiple documents reference deprecated Llama model Fix: Comprehensive audit and update
...
backend/ai-service-qwen.js
Update backend/server.js
Update backend/routes/civic-routes.js
...
No additional steps required (static hosting)
Verification Steps
Check health endpoint returns correct model
Test direct chat returns actual AI response
Verify frontend chat widget functions correctly
Confirm citations and sources display properly
