...
###############################################################################
# 📦 DEPLOY OPENSTATES FIX - v37.9.2 - COMPLETE FILE REPLACEMENT
# Workforce Democracy Project
# 
# Purpose: Fix OpenStates API 404 errors by deploying updated us-representatives.js
# Issue: State legislators returning 404 (only federal senators working)
# Fix: Updated jurisdiction format from "CO" to "ocd-jurisdiction/country:us/state:co/government"
# 
# This script:
# 1. Backs up current us-representatives.js
# 2. Replaces with fixed version
...
echo ""
echo "📋 Fix Details:"
echo "   • File: backend/us-representatives.js"
echo "   • Lines: 535-571 (OpenStates jurisdiction format)"
echo "   • Before: jurisdiction: 'CO' ❌"
echo "   • After: jurisdiction: 'ocd-jurisdiction/country:us/state:co/government' ✅"
echo ""
echo "═══════════════════════════════════════════════════════════════════"
echo ""
...
echo "📝 Writing fixed us-representatives.js..."
...
async function getStateLegislators(state, limit = 10) {
    try {
        ...
        const legislators = people
            .map(edge => formatStateLegislator(edge.node, state))
            .filter(leg => leg !== null)
            .slice(0, limit);
...
function formatStateLegislator(person, state) {
    try {
        const membership = person.currentMemberships && person.currentMemberships.length > 0 
            ? person.currentMemberships[0] 
            : null;

        if (!membership) return null;

        const chamber = membership.organization?.name || '';
        const district = membership.post?.label || '';
        const phone = person.contactDetails?.find(c => c.type === 'voice')?.value || null;
        const website = person.links && person.links.length > 0 ? person.links[0].url : null;

        return {
            id: `openstates_${person.id}`,
            name: person.name,
            title: chamber.includes('Senate') ? 'State Senator' : 'State Representative',
            office: chamber,
            level: 'state',
            party: person.partyName || 'Unknown',
            state: state,
            district: district,
...
# Verify the fix
echo "🔍 Verifying jurisdiction fix..."
if grep -q "ocd-jurisdiction/country:us/state:" us-representatives.js; then
    echo "✅ OpenStates jurisdiction format verified!"
else
...
echo "   • 5+ Colorado State Legislators"
echo "   • All with photos, contact info, websites"
echo ""
