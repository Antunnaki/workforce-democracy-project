...
1. **Created New Qwen AI Service**
   File: `backend/ai-service-qwen.js`
...
### Stability and Citation Enhancements (Dec 20, 2025)

5. **Resolved `ReferenceError: File is not defined`**
   File: `backend/utils/polyfills.js` (New), integrated into all services.
   Issue: Axios crashing in Node.js when processing multi-part data.
   Fix: Centralized polyfill utility added and imported in `server.js` and core services.

6. **Enhanced Frontend Chat Assistant**
   File: `js/modules/chat.mjs` and `js/chat-clean.js`
   Issue: Missing robust citation logic and markdown rendering in some modules.
   Fix: Ported superscript citations (¹ ² ³), collapsible sources, and markdown rendering.

7. **Added Diagnostic Tools**
   File: `backend/diagnose-ai-service.js` (New)
   Issue: Need for instant connectivity verification on VPS.
   Fix: Created script to test polyfills, API keys, and live AI responses.

8. **Secured Data Processing**
   File: `backend/routes/ai-bills-routes.js`
   Issue: Crashes during committee mapping when data was missing.
   Fix: Added `Array.isArray` safety checks for all bill data mappings.

### Secondary Fixes Needed

9. **Verify API Key Configuration**
   Issue: Actual Qwen API key needs to be configured on server
   Action: Add QWEN_API_KEY to server environment

10. **Update Frontend Configuration Flag**
    File: `js/config.js`
    Issue: GROQ_ENABLED flag may be misleading
    Fix: Consider renaming to AI_ENABLED or similar

11. **Restore Async Job Endpoints**
    Issue: Async endpoints may still have issues
    Investigation needed: Check route registration in `server.js`
...
3. Copy new/updated files:
   - `backend/utils/polyfills.js` (NEW - CRITICAL)
   - `backend/diagnose-ai-service.js` (NEW - DIAGNOSTIC)
   - `backend/ai-service-qwen.js`
   - `backend/article-scraper.js`
   - `backend/rss-service.js`
   - `backend/government-apis.js`
   - `backend/server.js`
   - `backend/routes/civic-routes.js`
   - `backend/routes/ai-bills-routes.js`
...
5. Run diagnostic on VPS:
   ```bash
   cd backend && node diagnose-ai-service.js
   ```
