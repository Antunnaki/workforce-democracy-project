...
| **Congress.gov API** | ✅ Working | Federal bills/legislators |
| **OpenStates API** | ✅ Working | State legislators, state bills |
...
```bash
# On VPScd /var/www/workforce-democracy
grep GROQ_API_KEY .env
```

---

### 2. OpenStates API ✅ CONFIGURED AND WORKING

**Purpose:** State legislators, state bills,state voting records  
**Status:**✅ Active and working (verified Dec 15, 2025)  
...
**Status:** ❌ Not configured (warningin PM2 logs)  
...
- Request free API key- Free for non-commercial/educational use
...
### 4. Congress.gov API (Federal Legislature)✅ CONFIGURED

**Purpose:** Federal bills, legislation, congressional votes  
**Status:** ✅ Now configured and working (updated Dec 15, 2025)  
**Used in:** `backend/government-apis.js`, `backend/us-representatives.js`

**Environment Variable:**
```bashCONGRESS_API_KEY=oTg6JWAJO6bAtUGeEmxCGLl52M94RJoe9v2Wks37
```
...
**Environment Variable (ifimplemented):**
```bash
...
### 9. FCC Area API (ZIP Code Lookup) ✅ NO KEY NEEDED**Purpose:** Convert ZIP codes to Congressional Districts  
...
**Noconfiguration needed!**

---

## How toCheck Your Current .env File
...
OPENSTATES_API_KEY=...   # Now configured!
```
...
# Addto .env: VOTESMART_API_KEY=your_key
```
...
1. **VoteSmart:** https://votesmart.org/share/api
...
**You should NO LONGER seethe warnings:**
```
⚠️ VoteSmart API key notconfigured
```
...
CONGRESS_API_KEY=oTg6JWAJO6bAtUGeEmxCGLl52M94RJoe9v2Wks37
OPENSTATES_API_KEY=7234b76b-44f7-4c91-a892-aab3ecba94fd
VOTESMART_API_KEY=your_actual_votesmart_key_here
...
curl https://api.workforcedemocracyproject.org/api/bills/HR1234

# Test AI chat (uses Groq)
curl https://api.workforcedemocracyproject.org/api/civic/llm-chat \
...
- ✅ Congress.gov API (federal bills and representatives)
- ✅ OpenStates API (state bills and representatives)

**Need to Add:**
- ⚠️ VoteSmart API (votingrecords)
...
1. **Sign up for VoteSmart API** (to enhance voting records)
2. **Add key to .env file** on VPS
