# 🚨 AGGRESSIVE FIX: Stop Publication Name Hallucination

## **THE SITUATION:**

Even though we added `[Truthout]` and `[Common Dreams]` to the prompt with explicit instructions, the LLM **STILL** writes:
- "According to Democracy Now..." ❌
- "The Intercept reports..." ❌  
- "ProPublica has investigated..." ❌
- "As reported by Jacobin..." ❌

**Why?** The LLM sees publication names like "Democracy Now, The Intercept, Jacobin, ProPublica" elsewhere in the system prompt (in the SOURCE FILTERING section) and thinks those are valid sources!

---

## **THE AGGRESSIVE FIX:**

1. **Remove ALL mentions** of Democracy Now, The Intercept, Jacobin, ProPublica from the system prompt
2. **Replace with generic descriptions** like "independent progressive sources" + "check [TRUSTED] tags"
3. **Rewrite citation rules** to be EXTREMELY explicit with step-by-step instructions

---

## **DEPLOYMENT COMMAND (Heredoc Method)**

Copy and paste this **ENTIRE BLOCK**:

```bash
cat > /var/www/workforce-democracy/AGGRESSIVE-FIX-HALLUCINATION.sh << 'EOFSCRIPT'
#!/bin/bash
###########################################
...
chmod +x /var/www/workforce-democracy/AGGRESSIVE-FIX-HALLUCINATION.sh
bash /var/www/workforce-democracy/AGGRESSIVE-FIX-HALLUCINATION.sh
```
...
### **1. Removes Publication Name Examples:**
...
```
❌ LLM sees these names and thinks they're valid sources!

**After:**
```javascript
// SOURCE FILTERING FOR PROGRESSIVE vs ESTABLISHMENT:
- If analyzing progressive vs establishment: Prioritize independent progressive sources over corporate media
- GENERAL PRIORITIES:
  1. Independent progressive sources (check [TRUSTED] tags)
  2. Court docs, OpenSecrets (primary sources)
  3. Investigative journalism sources (check [TRUSTED] tags)
```
✅ No specific publication names to confuse the LLM!

### **2. Extreme Citation Rules:**

**New instruction (step-by-step):**
```
- The sources are numbered [1], [2], [3], etc. at the top of this prompt
- Each source shows the publication name in BRACKETS: [Publication Name]
- When citing source [1], look at the BRACKETED name and use EXACTLY that name
- When citing source [2], look at the BRACKETED name and use EXACTLY that name
- EXAMPLE: If you see "[Truthout] Article Title..." then write "Truthout reports..."
- EXAMPLE: If you see "[Common Dreams] Article Title..." then write "Common Dreams reports..."
- DO NOT write "Democracy Now" unless you see [Democracy Now] in the source list
- DO NOT write "The Intercept" unless you see [The Intercept] in the source list
```
...
The LLM was confused because it saw "Democracy Now" and "The Intercept" in the SOURCE FILTERING section, so it thought those were valid sources to cite. By removing ALL specific publication mentions and using generic descriptions instead, the LLM has NO choice but to use the bracketed names from the actual source list.

---

**Ready to deploy? Copy the heredoc block above!** 🚀
