...
async function enrichWithDetails(organizations) {
    ...
    try {
                ...
                batch.map(async org => {
                    try {
                        ...
                        return {
                            ...
                            postal_code: orgData.zipcode || org.zipcode, // Alias for frontend
                            address: orgData.address || null,
                            street: orgData.address || null
                        };
                    } catch (error) {
                        console.warn(`⚠️ Failed to enrich ${org.ein}:`, error.message);
                        return org; // Return original if enrichment fails
                    }
                })
            );
        ...
        return organizations; // Return original data on error
    }
}
