 ...
 * 
 * LEVEL 3 DEEP RESEARCH: Comprehensive AI-powered bill analysis
 * 
 * Features:
 ...
 * - Amended bills: Invalidate cache, re-analyze
 * 
 * Philosophy:
...
router.post('/analyze', async (req, res) => {
    try {
        ...
        // Fetch bill-specific details based on level
        if (bill.level === 'federal') {
            ...
            const state = bill.state || 'NY'; // Default to NY for now
        ...
        console.log(`🧠 [AI Bills DEEP] Step 2: Building AI context...`);
        ...
        if (billDetails) {
            ...
            if (billDetails.committees && billDetails.committees.length > 0) {
        ...
        const prompt = `You are an expert legislative analyst providing comprehensive bill analysis for working people.
...
Your task is to provide DEEP RESEARCH analysis that empowers working people to understand this legislation's REAL-WORLD IMPACT.
...
2. **key_provisions**: Array of 5-7 key provisions. Focus on provisions that affect workers, wages, benefits, job security, or economic power. Be specific (cite section numbers if available).
...
4. **impact_score**: Rate impact on WORKING PEOPLE on 1-5 scale:
...
5. **impact_reason**: 2-3 sentences explaining the impact score. Cite specific evidence from the bill text, news articles, or data above.
...
8. **class_analysis**: NEW FIELD - Which economic class benefits most? Be specific: "Primarily benefits working-class families", "Shifts wealth to capital owners", "Mixed - helps professionals but not service workers". Explain WHY.

9. **arguments_for**: Array of 3-4 main arguments SUPPORTING the bill. INCLUDE arguments from labor unions, worker advocacy groups, and community organizations (not just politicians/corporations).
...
- If information is limited, acknowledge it (don't speculate)
- Prioritize information from official sources (bill text) over news articles
- If full bill text is available, extract actual provisions (section numbers, dollar amounts, dates)
...
Return ONLY valid JSON:
{
  ...
  "next_steps": "..."
        ...
        // Record cache miss metric (we just analyzed this bill)
        ...
        res.status(500).json({
            ...
            fallback: {
                summary: 'AI analysis temporarily unavailable. Please check the full bill text for details.',
                affects: 'Analysis pending',
                impact_score: 3,
                impact_reason: 'Unable to determine impact at this time',
                ...
                next_steps: 'Check bill status on official site'
            }
        });
    }
});
...
router.post('/chat', async (req, res) => {
    try {
        ...
        if (!bill || !question) {
            return res.status(400).json({ 
                success: false,
                error: 'Bill and question required' 
            });
        }
        ...
        const cachedAnswer = await billCache.getCachedQuestionAnswer(bill.id, question, 0.6); // 60% similarity threshold
...
- Help users understand how the bill might affect them or their community
...
Answer the user's question about this bill.`
        ...
        // V37.17.0: Cache this Q&A for future users
        ...
        console.log(`✅ [AI Bills Chat] Response sent and cached for ${bill.id}`);
        ...
        res.status(500).json({
            ...
            response: 'I apologize, but I\'m having trouble analyzing this bill right now. Please try again in a moment, or check the full bill text for details.'
        });
    }
});
