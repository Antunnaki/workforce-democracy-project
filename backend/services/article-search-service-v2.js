 ...
 * v37.20.4 - AGGRESSIVE apostrophe removal (all Unicode variants)
 * v37.20.3 - Smart quote possessive fix (handles both ' and ')
 * v37.19.0 - Fast local article search (replaces DuckDuckGo)
 */

console.log('🔥🔥🔥 ARTICLE-SEARCH-SERVICE-V2.JS LOADED - v37.20.4 🔥🔥🔥');
...
class ArticleSearchService {
    ...
    async searchArticles(keywords, options = {}) {
        ...
        try {
            ...
            // V37.20.1: Added debug logging to diagnose scoring issues
            let sources = articles.map(article => {
                ...
                // V37.20.6: BULLETPROOF FIX - Use simple regex to handle ALL possessives
                const words = searchQuery.toLowerCase().split(' ');
                
                // Log raw query for first article only
                const isFirstArticle = articles.indexOf(article) === 0;
                if (isFirstArticle) {
                    console.log(`  🔍 [v37.20.6] Processing query: "${searchQuery}"`);
                    console.log(`     Raw words: ${JSON.stringify(words)}`);
                }
                
                const keywords = words
                    .map(w => {
                        // STEP 1: Remove ALL possessive patterns (apostrophe + s at end)
                        // Matches: 's, 's, `s, ʼs (all apostrophe variants)
                        let cleaned = w.replace(/['ʼ`']s\b/g, '');
                ...
                if (isFirstArticle) {
                    console.log(`     Keywords after possessive removal: ${JSON.stringify(keywords)}`);
                }
                
                const titleLower = (article.title || '').toLowerCase();
                const excerptLower = (article.excerpt || '').toLowerCase();
                
                // DEBUG: Log first article scoring
                const isFirstArticle = articles.indexOf(article) === 0;
                if (isFirstArticle) {
                    console.log(`  🔍 [DEBUG] Scoring first article: "${article.title.substring(0, 50)}"`);
                    console.log(`     Query: "${searchQuery}"`);
                    console.log(`     Raw words: [${words.map(w => `"${w}"`).join(', ')}]`);
                    console.log(`     Keywords extracted: [${keywords.join(', ')}]`);
                    console.log(`     Title (lowercase): "${titleLower.substring(0, 60)}"`);
                    console.log(`     Initial score: ${relevanceScore}`);
                }
                ...
                if (isFirstArticle && personKeywords.length > 0) {
                    console.log(`     Person keywords detected: [${personKeywords.join(', ')}]`);
                }
                ...
                if (personKeywords.length > 0) {
                    personKeywords.forEach(personName => {
                        ...
                        if (titleLower.includes(personName)) {
                            if (isFirstArticle) console.log(`     ✅ Person name "${personName}" in title → +200`);
                        ...
                        // LARGE BOOST if person's name in EXCERPT
                            ...
                            if (isFirstArticle) console.log(`     ✅ Person name "${personName}" in excerpt → +100`);
                        ...
                        // If person's name is NOT in title/excerpt, this article is NOT about them
                            ...
                            if (isFirstArticle) console.log(`     ❌ Person name "${personName}" NOT in title/excerpt → -50`);
                ...
                const finalScore = Math.min(200, Math.max(0, relevanceScore));
                
                // DEBUG: Log first article final score
                if (isFirstArticle) {
                    console.log(`     Final score (after cap): ${finalScore}`);
                }
                ...
                return {
                    ...
                    // V37.20.1: Add trusted flag for Democracy Now, Intercept, Drop Site
                    trusted: ['Democracy Now', 'The Intercept', 'Drop Site News', 'Common Dreams', 'ProPublica'].includes(article.source)
                };
            });
            
            // V37.20.1: Old return block removed (was duplicate)
