    ...
    async searchArticles(keywords, options = {}) {
        ...
        try {
            ...
            let sources = articles.map(article => {
                ...
                // V37.20.1: Remove possessive 's FIRST, THEN punctuation (fixes "mamdani's" → "mamdanis" bug)
                // V37.20.3: Handle BOTH regular apostrophe (') AND smart quote (') for possessives
                const keywords = searchQuery.toLowerCase()
                    .split(' ')
                    .map(w => w.replace(/['']s\b/g, '')) // Remove possessive 's with BOTH ' and ' ("mamdani's" OR "mamdani's" → "mamdani")
                    .map(w => w.replace(/[^a-z0-9]/g, '')) // THEN remove all punctuation
                    .filter(w => w.length > 3);
