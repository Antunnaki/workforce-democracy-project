...
async function queryBackendAPI(chatType, query, options = {}) {
   ...
   try {
        ...
        // V37.0.2: Format conversation history forLLM (convert to role/content format)
...
conversationHistory: formattedHistory // ✅ Properly formatted history
        ...
        if (!response.ok) {
            ...
            throw new Error(`BackendAPI error: ${response.status} ${response.statusText}`);
        }
...
responseLength: (data.message || data.response || '').length
        });
        
        // V37.0.2: Save conversation to history
        const responseText = data.message || data.response || 'No response content';
        saveConversationHistory(chatType, query, responseText);
        
        return {
            success: true,
           response: responseText,
            message: responseText,  // Added for consistency
            source: 'llm',
            cost: 0,
...
/**
 * Get conversation history for a chat type
 */
function getConversationHistory(chatType) {
    ...
    try {
        ...
        console.warn(`[Backend API] Failed to load chat history for ${chatType}:`, error);
        return [];
    }
}
