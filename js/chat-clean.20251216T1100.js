/*WDP chat-clean.20251216T1100.js LIVE v37.9.15-FINAL builtAt=2025-12-16T11:00Z TIMESTAMP-VERIFIED */
 ...
 * - Citations displayas `_CITATION0_` instead of clickable links
 ...
 * 
 * CRITICAL FIX: Use DOM manipulation instead of innerHTML string replacement* to prevent HTML escaping issues
 */
 ...
 * 
 *CRITICALFIX: Do NOTwrap in <p> tags - we'll add those AFTER citations
...
// V37.18.12: FIX - Don't split on '. ' if it's part of a numberedlist (1. 2. 3. etc.)
...
// Determine paragraph grouping based on content length
    ...
    for (let i = 0; i < sentences.length; i += paragraphSize) {
        constgroup = sentences.slice(i, i + paragraphSize);
...
if (!directResponse.ok) {
       ...
       const data = await directResponse.json();
        ...
        console.log('[CleanChatv37.9.12-ASYNC] ✅ Received result after', elapsedTime, 'seconds:', data);
...
console.warn('[CleanChat] ⚠️aiResponse is not a string, converting:', typeof aiResponse);
        ...
        // VALIDATION: Check forcitation/source mismatch
        if (citationCount > 0 && sources.length !== citationCount) {
            ...
            if (citationCount > sources.length) {
                ...
                console.error(`   → Citations [${sources.length + 1}] through [${citationCount}] will be REMOVED fromdisplay`);
                ...
                console.error(`   → Check LLM prompt: Should only add citations whensources exist`);
            } else {
...
window.toggleInlineChat = function(chatId) {
    ...
    constchatWindow= document.getElementById(windowId);
...
//FIX v37.9.14: Setcurrent container so displayAIResponse knows where to write
   ...
   // Add user message to UI
    ...
    //Add loading messagewith animated dots
    const loadingDiv = document.createElement('div');
...
const button = document.getElementById('floatingChatButton');
    constwindow = document.getElementById('floatingChatWindow');
...
console.log('  ✅ localStorage persistence (survives tab switch)');
    ...
    // FIX v37.9.8: Load chat history from localStorage on page loadloadChatHistory();
...
// ChatWidget forfloating chat functionality
