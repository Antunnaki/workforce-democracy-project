...
export function mount(target = document.getElementById('app-root')){
  ...
  btn.addEventListener('click',()=>{ modal.style.display='block'; setTimeout(()=>input.focus(),0); });
  closeBtn.addEventListener('click',()=>{ modal.style.display='none'; });
  form.addEventListener('submit', async (e)=>{
    e.preventDefault();
    const val = input.value.trim(); if (!val) return; input.value='';
    const user = document.createElement('div'); user.textContent = val; user.style.cssText='align-self:flex-end;background:#1f2a44;padding:10px 12px;border-radius:10px;max-width:90%'; msgs.appendChild(user); msgs.scrollTop = msgs.scrollHeight;
    try {
      const r = await fetch(`${CONFIG.API_BASE}/api/chat`, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({message:val})});
      const data = r.ok ? await r.json() : {reply:`Error ${r.status}`};
      const ai = document.createElement('div'); ai.textContent = data?.reply || data?.message || '[No reply]'; ai.style.cssText='align-self:flex-start;background:#122233;padding:10px 12px;border-radius:10px;max-width:90%'; msgs.appendChild(ai); msgs.scrollTop = msgs.scrollHeight;
    } catch(err){
      const ai = document.createElement('div'); ai.textContent = 'Network error. Please try again.'; ai.style.cssText='align-self:flex-start;background:#122233;padding:10px 12px;border-radius:10px;max-width:90%'; msgs.appendChild(ai);
    }
  });
